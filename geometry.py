...
class Geometry:
    ...
    def new_RMSD(self, ref, align=False, heavy_only=False,
                 targets=None, ref_targets=None, sort=False):
        def get_eig(R):
            F = np.zeros((4, 4), dtype=np.double)
            ...
            vals, vecs = np.linalg.eigh(F, UPLO='U')
            print('eig', vals, vecs.T, sep='\n')
            for i in range(-1, -5, -1):
                val = vals[i]
                vec = vecs.T[i]
                if np.abs(np.linalg.norm(vec[1:]) - 1) < 10**-8:
                    break
            return val, vec[0], vec[1:]

        def get_val(R, targets, ref_targets):
            x0 = sum([np.linalg.norm(t.coords)**2 + np.linalg.norm(r.coords)**2
                      for t, r in zip(targets, ref_targets)]) / 2
            K0 = (R[0, 1]**2 + R[0, 2]**2 - R[1, 0]**2 - R[2, 0]**2)**2
            K0 += (-R[0, 0]**2 + R[1, 1]**2 + R[2, 2]**2
                   + R[1, 2]**2 + R[2, 1]**2
                   - 2*(R[1, 1]*R[2, 2] - R[1, 2]*R[2, 1])) \
                * (-R[0, 0]**2 + R[1, 1]**2 + R[2, 2]**2
                   + R[1, 2]**2 + R[2, 1]**2
                   + 2*(R[1, 1]*R[2, 2] - R[1, 2]*R[2, 1]))
            K0 += (-(R[0, 2] + R[2, 1])*(R[1, 2] - R[2, 1])
                   + (R[0, 1] - R[1, 0])*(R[0, 0] - R[1, 1] - R[2, 2])) \
                * (-(R[0, 2] - R[2, 1])*(R[1, 2] + R[2, 1])
                   + (R[0, 1] - R[1, 0])*(R[0, 0] - R[1, 1] + R[2, 2]))
            K0 += (-(R[0, 2] + R[2, 0])*(R[1, 2] + R[2, 1])
                   - (R[0, 1] + R[1, 0])*(R[0, 0] + R[1, 1] - R[2, 2])) \
                * (-(R[0, 2] - R[2, 0])*(R[1, 2] - R[2, 1])
                   - (R[0, 1] + R[1, 0])*(R[0, 0] + R[1, 1] + R[2, 2]))
            K0 += ((R[0, 1] + R[1, 0])*(R[1, 2] + R[2, 1])
            ...
            K0 += ((R[0, 1] + R[1, 0])*(R[1, 2] - R[2, 1])
                ...
                * (-(R[0, 1] - R[1, 0])*(R[1, 2] + R[2, 1])
                   + (R[0, 2] - R[2, 0])*(R[0, 0] + R[1, 1] - R[2, 2]))
            K1 = -8*np.linalg.det(R)
            K2 = -2*np.sum(R**2)
            k, s, x = 0, 1, x0
            while s > 10**-15 and k < 500:
                X2 = x**2
                a = K2 + X2
                F = a*X2 + K1*x + K0
                dF = K1 + 2*x*(a + X2)
                if dF < 10**-12 and F < 10**-12:
                    break
                s = F/dF
                x -= s
                k += 1
            print(k)
            return np.sqrt(2*np.abs(x - x0) / len(targets))
        ...
        def _RMSD(targets, ref_targets):
            print(targets)
            print(ref_targets)
            R = self.coords(targets).T @ ref.coords(ref_targets)
            sum_mags = 0
            count = 0
            for x, y in zip(targets, ref_targets):
                sum_mags += (x.coords @ x.coords)
                sum_mags += (y.coords @ y.coords)
                count += 1
            val, ang, vec = get_eig(R)

            if sum_mags - 2*val > 0:
                rmsd = np.sqrt(sum_mags - 2*val) / count
            else:
                rmsd = 0
            print('quat', ang, vec)
            print('val', get_val(R, targets, ref_targets))
            print('rmsd', rmsd)
            return rmsd, ang, vec

        ref = ref.copy()
        # get target atoms if provided
        tmp = targets
        if targets is not None:
            targets = self.find(targets)
        else:
        ...
        if sort:
            orders = get_orders(self, targets)
            ref_targets = [a for a in ref.reorder(
                canonical=False)[0] if a in ref_targets]
        ...
        print(ref.atoms[2])
        if np.linalg.norm(vec) > 0 and align:
            print(self.atoms[2])
            self.rotate(vec, ang)
            print(self.atoms[2])
            self.coord_shift(ref_com)
