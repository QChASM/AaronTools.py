...
class Component(Geometry):
    ...
    def __init__(
        self, structure, name="", comment="", tag=None, to_center=None
    ):
        """
    ...
    def get_frag_list(
        self, targets=None, max_order=None, refresh_connected=False
    ):
        """
    ...
    def detect_backbone(self, to_center=None):
        ...
        for frag_tup in frag_list:
            ...
            sub_atoms = sub_atoms | set(sub.atoms)
        ...
        for a in new_tags:
            ...
            if len(tags) > 1:
                ...
                if max_length < 0:
                    ...
                    min_length = min(sub_length)
                    if min_length < 0 and -min_length > max_length:
                        keep = sub_length.index(min_length)
                        a.add_tag(tags[keep])
        ...
        for a in self.atoms:
            ...
            remove = []
            for t in a.tags:
                if t in Substituent.cache["lengths"].keys():
                    remove += [t]
            for r in remove:
                a.tags.discard(r)
            a.add_tag("backbone")
