...
follow_parser = argparse.ArgumentParser(description='move the structure along a normal mode', \
    formatter_class=argparse.RawTextHelpFormatter)
follow_parser.add_argument('input_file', metavar='input file', \
                            type=str, \
                            nargs=1, \
                            help='input frequency file (i.e. Gaussian output where "freq" was specified)')

follow_parser.add_argument('-m', '--mode', \
                            type=str, \
                            nargs='+', \
                            required=False,\
                            default = None, \
...
follow_parser.add_argument('-r', '--reverse', \
                            action='store_const', \
                            const=True, \
                            default=False, \
                            required=False, \
...
follow_parser.add_argument('-a', '--animate', \
                            type=int, \
                            nargs=1, \
                            default=None, \
                            required=False, \
...
follow_parser.add_argument('-o', '--output-destination', \
                            type=str, \
                            nargs='+', \
                            default=False, \
                            required=False, \
...
if args.mode is None:
    modes = [[i] for i, freq in enumerate(G_AAron_file.other['frequency'].data) if freq.frequency < 0]
else:
...
if len(outfiles) != len(modes):
    warn("number of output files does not match number of modes: %i files, %i modes" % (len(outfiles), len(modes)))
