...
from AaronTools.const import QCHASM, AARONLIB, HOME
...
submit_parser = argparse.ArgumentParser(description="submit a QM computation to the queue", \
    formatter_class=argparse.RawTextHelpFormatter)
submit_parser.add_argument("infile", metavar="input file",
                           type=str,
                           nargs="+",
                           help="a Psi4, ORCA, or Gaussian input file",
)

submit_parser.add_argument("-d", "--config-default",
                           type=str,
                           default=None,
                           dest="section",
...
submit_parser.add_argument("-j", "--job-template",
                           type=str,
                           default=None,
                           dest="template",
...
submit_parser.add_argument("-p", "--processors",
                           type=int,
                           required=False,
                           default=None,
                           dest="processors",
...
submit_parser.add_argument("-m", "--memory",
                           type=int,
                           required=False,
                           default=None,
                           dest="memory",
...
submit_parser.add_argument("-t", "--walltime",
                           type=int,
                           required=False,
                           default=None,
                           dest="time",
...
submit_parser.add_argument("-wl", "--wait-last",
                           action="store_true",
                           default=False,
                           dest="wait_last",
...
submit_parser.add_argument("-we", "--wait-each",
                           action="store_true",
                           default=False,
                           dest="wait_each",
...
for i, f in enumerate(args.infile):
    ...
    if args.section is not None:
        if args.processors is None:
            processors = config.getint(args.section, "processors", fallback=None)
    ...
    submit_process = SubmitProcess(f, walltime, processors, memory, template=template)
